"""
Scraper do Diário Oficial Eletrônico de Piracicaba, SP.

Portal: https://www.piracicaba.sp.gov.br/diario-oficial-eletronico/
Padrão: página lista edições por data; links apontam para PDFs nomeados com a data.
"""
import re
import sys
...
# Ajuste caso o portal mude de estrutura
_PORTAL_URL = "https://www.piracicaba.sp.gov.br/diario-oficial-eletronico/"
_HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; licitacoes-ia/1.0)"}


class PiracicabaScraper(BaseScraper):
    ...
    def buscar_edicoes(self, data: date) -> list[DiarioOficial]:
        ...
        links_pdf = _filtrar_links_pdf(soup, data)
...
def _filtrar_links_pdf(soup: BeautifulSoup, data: date) -> list[str]:
    """
    Procura todos os <a href="...pdf"> na página e filtra pelos que
    correspondem à data (DD/MM/YYYY ou DDMMYYYY no nome do arquivo).
    """
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not href.lower().endswith(".pdf"):
            continue
        # Aceita: data no href OU texto do link contém a data
        texto_link = a.get_text(strip=True)
        if padrao_data.search(href) or padrao_data.search(texto_link):
            url = href if href.startswith("http") else f"https://www.piracicaba.sp.gov.br{href}"
            candidatos.append(url)

    # Fallback: se nenhum link com data, pega todos os PDFs da página
    # (útil quando o portal não codifica datas nos nomes)
    if not candidatos:
        for a in soup.find_all("a", href=True):
            href = a["href"]
...
def _baixar_e_processar(
    ...
    print(
        f"[{municipio}] OK — {len(texto)} chars, {len(do.trechos)} trechos de licitação, OCR={usou_ocr}",
        file=sys.stderr,
    )
    return do
